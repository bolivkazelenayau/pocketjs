...
pub struct AppConfig {
    ...
    /// Requested static MSAA sample count. Product-specific preferences are
    /// translated to this generic renderer value at the application boundary.
...
impl WindowState {
    fn configure_surface(&self) -> Result<()> {
        ...
        if let Some(direct_composition) = &self.direct_composition {
            // The wgpu DX12 backend creates/replaces the composition swapchain
            // and calls IDCompositionVisual::SetContent during configure.
...
impl DirectCompositionState {
    ...
    fn create_surface(&self, instance: &wgpu::Instance) -> Result<wgpu::Surface<'static>> {
        ...
        // COM refcount, and WindowState also retains this state until after the
        // surface is dropped.
        unsafe {
